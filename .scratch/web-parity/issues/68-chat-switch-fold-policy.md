...
2026-09-20 — Implemented on `wp-fu/68` (branch off `web-parity/followup` @ 3a70aab5), two commits split by client. **Web:** `ToolGroupMotionStore.captureExplicitFolds/restoreExplicitFolds` (pins only — no tween/disclosure clocks, no reveals, no blobs), new `state/transcript-fold-state.ts` (module-scoped `TranscriptFoldCache`, bounded per-engine LRU keyed `(engineKey, docId)`, `MAX_SAVED_FOLD_CHATS = 32`, empty capture never overwrites, no disk persistence), the surface restores pins during the render that creates the motion store (before the first rows render, before the scroller's viewport-restore layout effect, so restore precedes the height estimates; a StrictMode-safe re-apply in the mount effect undoes the simulated remount's reset), captures at teardown before reset; `EngineClient` exposes its registry `engineKey` (null for unkeyed clients, which share a `""` bucket). Subagent (`alignTop`) surfaces are excluded, mirroring the desktop's doc-override instances which never attach. A restored closed pin overrides autoOpen/arrivalPending through ticket 70's shared `toolGroupGeometry` resolver — no second resolution path. **Rust:** `SavedFoldPins`/`SavedFoldCache` (same LRU discipline as `SavedViewportCache`, `MAX_SAVED_FOLD_CHATS = 32`), `remember_current_folds` capture (clocks stripped) in the `sync` selected-chat branch with the viewport's eviction-safe ordering (read incoming before the outgoing capture), `tool_details.clear()` on attach so the LRU is the single cross-chat memory, and settled-pin restore before rows are derived; the quick deselect-retain path and `finish_route_exit` stay consistent (the retain branch never consults the cache). Recorded capacity sub-choice: 32 chats per engine on both clients (the decision record left the number open; pins are tiny and 32 covers navigation while bounding memory). **Tests:** web `tests/transcript-model.test.ts` (now jsdom) adds the required real-switch regressions — `explicit_fold_choices_survive_real_chat_switch` and `restored_folds_do_not_replay_arrival_motion` mount the real `TranscriptView`/`TranscriptStore` against two chat identities and two engines, unmounting between visits (A→B→A with fresh stores/surfaces — never `sync(true)` on one retained store), and assert pins restored settled, pin-over-autoOpen through the resolver, no cross-chat/cross-engine leaks, no manufactured pins, restore-before-snap ordering, no reveal/shimmer restarts, and genuinely live arrivals still staggering; Rust adds the same two names (real attaches through `sync`) plus `fold_pin_cache_is_bounded_and_recency_protected`. Evidence: `pnpm -r build` (web) passes; `pnpm test` in `web/packages/app` — 1498 passed / 91 files (96 in `transcript-model.test.ts`, incl. the 3 retained baseline tests); `cargo check -p roboco-ui` clean; `cargo test -p roboco-ui` — 941 passed / 0 failed / 5 ignored, with the 3 pre-existing desktop navigation tests green. **Pending:** all runtime/visual acceptance in §6 (paired screenshots and motion captures for the named states) remain explicitly pending — no dev server, browser, or desktop app was run per the hard rules; the jsdom regressions prove state/geometry only, never painted behavior.
