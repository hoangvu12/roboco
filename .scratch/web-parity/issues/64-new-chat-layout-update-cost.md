...
2026-09-20 (wp-fu/64): Code work landed; runtime evidence PENDING (no browser may be started from this session).

Landed:
- `chat-page.tsx`: new exported `ColumnWidthPublication` policy — every column ResizeObserver tick updates the measured width and the clamped live composer target (`composerWidthTargetRef`, the dock pump's per-frame read) with no React commit; `setColumnWidth` defers only on the blank canvas while `sidebarTweenSignal` is active and publishes the current final measurement once on the signal's settle edge (transitionend / cap / drag / reduce disarms all funnel through `settle()`). Selected-chat publication (QueuePanel's `composerWidth`) stays responsive. The render-time `composerWidthTargetRef.current = composerWidthTarget` assignment is removed — render can no longer overwrite the live dock target with the stale published width. `liveAvailableWidth={composerWidthTargetRef}` passed to Composer.
- `composer.tsx`: optional `liveAvailableWidth` ref prop read by the evaluate pass for the strip width budget (live ref → prop-parked ref → 768 cap); unchanged-layout bailout — `setLayout` returns prior state only when every `PillLayout` field is equal (`pillLayoutEquals`). Evaluator internals otherwise untouched (ticket 74's region).
- `titlebar.tsx`: `useIslandTween`'s per-frame `setPainted`/`setPump` React loop replaced by exported `IslandTweenDriver` — one imperative rAF loop over a `{from,to,startedAt}` tween ref evaluating `evalWidthTween` (200ms RESIZE ease-out), writing top/height/opacity to the island element before paint; reversal re-arms from the painted progress; initial presentation settled (no entrance); reduced motion snaps the endpoint + gate synchronously, even with an unchanged target; dispose cancels the pending frame. React holds only the boolean mount gate (panel iff painted p > 0.001, published on crossings). Ticket 66's horizontal geometry helper/values, the mount gate, and the target predicate are untouched; `app.css` untouched (CSS interpolation not chosen). Phone safe-area displacement stays in the cluster CSS.
- Tests: `titlebar-island.test.ts` +8 (driver on a manual clock/frame queue: settled mount, 200ms timing via the real `evalWidthTween`, reversal-from-painted, reduced-motion snap mid-tween / unchanged-target / with flip, dispose, no re-arm restart); `sidebar-tween.test.ts` +5 (publication policy: deferred glide ticks + one settle flush, reversal, drag takeover, selected-chat responsive, redundant-tick/round-trip no-op); `dock-glide.test.ts` +1 (observer-fed live target riding inside a 420ms dock glide: zero width publications during the armed window, pump reads the live ref per frame, mount-crossing + settle state writes stay ≤ 3).
- `pnpm -r build` (web/): pass, exit 0. `pnpm test` (web/packages/app/): 89 files / 1402 tests passed, including the known-flake-prone `tests/registry.test.ts` (no flake observed this run).

Pending runtime evidence (trace plan, per §2.5 steps 1/5 and §2.6): capture the baseline and after traces of the sidebar toggle on the new-chat canvas in an authorized running app — 1440px window AND a narrower desktop window, empty + multiline composer — recording viewport, sidebar width, composer text, effects, reduced-motion setting, page commits, observer counts, and main-thread layout/paint work, with matched desktop/web recordings (intermediate frames, not endpoint screenshots). Also pending: canvas↔chat dock-phase timing above/below the 768px cap, rapid reversal → seam-drag takeover on screen, reduced-motion mid-motion flip, viewport resize and typing during the glide.

What the traces still need to establish (honest): (a) that whole-page commits in the controlled scenario no longer scale with animation frames — the code/tests prove the publication policy, not the measured commit counts; (b) which remaining work (hero remask on settle — ticket 65's bitmap cost, bottom-stack measurement per commit, CSS transition paint) now dominates the frame budget; (c) that no regression snuck into the narrow-window case: below the 768px composer cap the pill width now tracks at the settle publish rather than per frame (endpoint publication by design per §2.1), so the pill stays centered-but-static during the 200ms glide and converges at settle — verify against the desktop recording; (d) island visual continuity across reversal/reduced-motion on real paint (driver behavior is unit-tested; raster evidence is not captured).
