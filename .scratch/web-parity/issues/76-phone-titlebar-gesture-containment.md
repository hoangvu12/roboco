...
**2026-09-20 — implementation landed (worktree `fu-76`, branch `wp-fu/76`):** `web/packages/app/src/styles/app.css` — the phone `@media (max-width: 768px)` `.titlebar` rule (the existing ticket-50 safe-area block) now declares `touch-action: none`, so no direct-manipulation gesture starting on the app titlebar may pan the browser; per W3C Pointer Events 3 §8.2 the ancestor intersection covers the cluster/trailing controls, so no per-button overrides were added (step 3 verified against `titlebar.tsx`: plain `<button onClick>` controls, no touch handlers, hit regions are the buttons themselves — component unchanged, read-only as specified). The three misleading manipulation comments (base `.titlebar`, `.window-control`, `.header-icon-button`) now state that manipulation permits panning and kills only double-tap-zoom, naming the scoped `none` as the pan guard. Desktop-width rules unchanged (base `.titlebar` keeps `manipulation`); no geometry, island/cluster, safe-area or transcript/drawer/body touch policy touched. New contract test `web/packages/app/tests/phone-titlebar-gestures.test.ts` (4 tests, follows the repo's `readFileSync(app.css)` idiom): phone `.titlebar` carries `none` on its safe-area rule exactly once, desktop `.titlebar` keeps `manipulation`, `.transcript` and `body` declare no `touch-action`. Verification: `pnpm -r build` from `web/` — pass (tsc + vite); `pnpm test` from `web/packages/app/` — 87 files / 1365 tests passed. **Pending device evidence (acceptance rows 3–5):** no authorized phone browser was available and no dev server/browser may be started from the implementing agent, so the runtime matrix (hold/drag on title text, blank chrome and each control; drawers open; keyboard up/down; 768/769px boundary; OS status-bar control case, with document/transcript/visualViewport offset recordings) is explicitly pending. The CSS declaration proves the guard value and scoping only — if the reported hold symptom persists on device, the residual trigger (visual-viewport pan vs. transcript compensation vs. OS chrome) still needs measurement per §2.4 steps 4–5 before any further change.
