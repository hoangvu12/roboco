...
**2026-09-20 (wp-fu/72, commit b612f85b):** Implemented. `RightPane` now arms a transient, owner-keyed expanded-width hold on the expanded-open → closed edge (`usePhoneExpandedCloseHold`; pure decision core `armsExpandedCloseHold` / `releasesExpandedCloseHold` exported for the node tests) and carries `right-pane-closing-expanded` on the aside until the drawer's transform settles — guarded `transitionend`/`transitioncancel` (target = aside, property = transform), a `prefers-reduced-motion` change listener for the mid-close flip, and a MENU_IN(140ms) + 100ms fallback clock. app.css adds `.right-pane.right-pane-closing-expanded` to the phone-only `width: 100vw !important` selector (no token, timing, z-order or tab-placement changes). Release paths: reopen, owner switch, 768px breakpoint flip, reduced motion; route unmount dies with the component. Logical `open`/`expanded` still reset in the close commit — the pane store gained no state (shape asserted in the regression test). Desktop glide, `data-pane-snap` and desktop width animation untouched. Verified: `pnpm -r build` from `web/` passes (tsc + vite); `pnpm test` from `web/packages/app/` passes — 87 files / 1371 tests, incl. new `phone-pane-close.test.ts` (9 tests: logical-vs-presentation independence, arm/release truth tables, interruption paths) and the new `close_resets_logical_flags_immediately` regression in `right-pane.test.ts` (15 tests total there). Skipped/pending: the §2.5 runtime/visual matrix is PENDING — no dev servers, browsers or `web_smoke` were run (hard rule); real-phone recordings of the expanded close (toggle/backdrop/Escape), normal close/reopen, rapid reopen + owner change + resize across 768px, and reduced-motion (before and mid-close) still need capture, with intermediate frames distinguishing container slide from the intentional Files content unmount. Acceptance boxes 1, 3 and 4 rest on the unit-level decision-core coverage until that evidence lands.
