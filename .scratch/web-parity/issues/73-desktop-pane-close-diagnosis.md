...
2026-09-20 (wp-fu/73): Evidence artifact delivered at `../research-2026-09-20/desktop-pane-close-reproduction.md`. Source-level diagnosis is complete against `37c354ff` (cited files verified unchanged at HEAD `7389969b`). Conclusions: pane outer width, held inner width, titlebar band, conversation reflow, suppression flags (`data-pane-snap` / `data-rb-resizing` / reduced motion) and reversal handling are all source-equivalent to the desktop contract — no ordinary same-chat desktop close defect is established. The one confirmed behavior is intentional parity: Files/File content blanks on frame 0 of a close in BOTH clients (right-pane.tsx:73-89; shell.rs:6469-6476) — gap 73b, a product decision if a change is wanted. Rejected at source: data-pane-snap on same-chat toggle, frame-0 unmount, inner squeeze, independent band snap, takeover-close content snap; ticket 72's phone bug excluded as desktop evidence. Pending runtime evidence (no authorized runtime in this task): M1 same-chat width continuity capture, M2 Files unmount-vs-glide timing, M3 takeover/chat-switch traces, M4 rapid-reversal retargeting, M5 reduced-motion/resize-flag state, M6 frame-time stall trace — each with the exact measurement named in the artifact §1. Recommendation: one bounded capture session, then either close as "explained by designed Files policy" or open a channel-scoped implementation ticket from an actual trace. No production code, tests or app state changed.

2026-09-20 (coordinator): Conditional product question resolved by the user — current behavior accepted ("it's fine, not that big a deal"). No Files-close resource/retention policy change; the blanking stays as desktop-matching intentional parity. Ticket closes on the diagnosis; the pending runtime captures in the artifact remain pending but no longer gate a follow-up.
