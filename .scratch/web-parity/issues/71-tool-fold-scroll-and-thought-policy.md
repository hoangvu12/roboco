...
**2026-09-20 evening � live crash found and fixed in the C path (render-side arm).** The follow-up build crashed repeatedly in live use: `RefCell already mutably borrowed` at zui `list.rs:632` (`ListState::logical_scroll_top`), backtrace `render_tool_group` ? `arm_automatic_tool_fold_scroll` (transcript.rs:4855) ? the `List` prepaint's `borrow_mut`. The ticket-71 C implementation called the arm from inside the render pass � reading `ListState` while the list's own layout holds it � exactly the discipline the file documents elsewhere ("neither operation reads the borrowed ListState"). Trigger: any group that auto-closes between two renders (routine while an agent streams) with the one-owner gate passing. Fixed in `67a54ce6` (`fix(ui): ticket 71 defer render-side auto-fold arm out of list prepaint`): the detection sets `pending_auto_fold_arm` only, and `render` drains the flag before building the list element (one frame after detection; the tween's canvas repaint driver guarantees that frame), so the arm's `ListState` reads run outside the borrow. `cargo test -p roboco-ui` 944/944 after extending `live_thought_streams_open_and_settles_closed` with the deferral contract (aged reveal starts drive a real render-side auto-close; flag set in-render, drained outside, gate refusal while another compensator is live). A gpui test that drives a real `List` prepaint with the borrow held remains a known gap � the headless suite cannot hold the RefCell from outside `ListState`.
