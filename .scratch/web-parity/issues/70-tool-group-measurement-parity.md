...
**2026-09-20 — implemented on `wp-fu/70`.** Landed: new `web/packages/app/src/lib/tool-group-geometry.ts` holds the ONE pure resolver (`toolGroupGeometry`, plus moved `effectiveToolDetail`/`effectiveToolAffordance` and the fold `tweenHeight`) behind a read-only `ToolGroupGeometryState` view; `ToolGroupRow` consumes it, and `estimateRowHeight`'s `toolGroup` branch delegates to it (rail rows now 32 px: open one/three plain tools estimate 60/124, closed 26; standalone spawn cards keep `chipsHeight`/38 px, `CHIP_HEIGHT` untouched). The estimator's `groupFoldOpen` callback became a `ToolGroupEstimateContext` (read-only state + ONE caller timestamp + reduced flag) threaded through the prefix sums, `captureAnchor`, and `readingTopRow`. Cache boundary per §2.4: the scroller recomputes per-row semantic keys each render (`computeToolMeasurementKeys`), tags ResizeObserver measurements with them, and `pruneStaleToolMeasurements` drops only a tool row whose stored key no longer matches (e.g. an unmounted row's in-flight blob fetch completing); a motion-store subscription re-renders the scroller only when some tool row's key actually moved. No markup, auto-open, arrival, reservation, or fold policy changed; tool-motion.ts untouched. Verified against the desktop that rail spawn links painting at 38 px while counted 32 in the analytic body matches `subagent_chip` (`crates/ui/src/transcript.rs:7402`) — 1:1, no defect. Tests (`web/packages/app/tests/transcript-model.test.ts`): replaced the self-confirming chipsHeight expectations with independent 60/92/124/26 totals; added `rail_group_estimate_matches_rendered_geometry`, `group_geometry_resolves_pins_arrivals_and_effective_details` (pins, default/pinned detail, fetched payload+affordance, arrival-only open incl. future starts, mid-tween timestamp, reduced motion), `fetched_detail_geometry_is_read_only_and_respects_request_recency` (real store, deferred completion, recency click without re-fetch, no version/order churn from reads), `late_blob_completion_invalidates_only_affected_tool_measurement`, and the controller/geometry-level `unmeasured_rail_group_does_not_prematurely_fill_runway` (arithmetic only). `pnpm -r build` (web/) and `pnpm test` (web/packages/app/: 89 files, 1392 tests) pass; no Rust changes. **Pending runtime evidence (needs a browser; not gathered per subagent constraints):** the §6 capture matrix (desktop/web screenshots + motion captures, A→B→A remount, 500 ms late-measurement correction, own-send hold near fill, retired-runway escape) and any claim that the removed 6 px/tool excess actually retired a runway early at runtime — the unit test proves the arithmetic only.
