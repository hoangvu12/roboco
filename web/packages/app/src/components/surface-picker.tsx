 ...
 * clipped in narrow panes). Each row opens a surface; Files is
 * single-instance, every other row mints a tab.
 */
 ...
 * (desktop-only: no embedded web view on web). The `+` menu's rows are the
 * same list (`shell.rs:7136-7143`).
 */
export const SURFACE_CHOICES: readonly SurfaceChoice[] = [
  {
    id: "surface-card-files",
    icon: "folderWithFiles",
    label: "Files",
    needsGit: false,
    open: (chatId) => rightPaneStore.addFilesSurface(chatId),
  },
  {
