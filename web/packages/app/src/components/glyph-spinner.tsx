 ...
 * color.
 *
 * The pulse loaders (`RobocoMarkLoader`, `RobocoLoader`) share the 2.4s
 * `ROBOCO_PULSE` cosine (opacity 0.08→1, scale 0.9→1) with per-cell stagger
...
// ---------------------------------------------------------------------------
// The roboco pulse loaders (`ROBOCO_PULSE` = 2400ms, proto/motion.rs:16)
// ---------------------------------------------------------------------------

/** `ROBOCO_PULSE_MS` (proto/motion.rs:16). */
const ROBOCO_PULSE_MS = 2400;
...
/** `ROBOCO_CELLS` (proto/motion.rs:21) — the wave loader's cell count. */
export const ROBOCO_CELLS = 5;
/** `MARK_SPREAD` (proto/motion.rs:108) — the mark's sweep window. */
export const MARK_SPREAD = 0.55;

/**
 * `MARK_CELLS` (proto/motion.rs:99-105) — the roboco mark's pixels, `[x, y]`
...
export interface RobocoMarkLoaderProps {
...
/**
 * `roboco_mark_loader` (loaders.rs:30-67): the full logo pixel grid with a
 ...
 */
export function RobocoMarkLoader({ height = 140, className }: RobocoMarkLoaderProps) {
        ...
        >
          <span
            ...
            style={{
              ...
              animationDelay: `${-markCellStagger(x, y) * ROBOCO_PULSE_MS}ms`,
            }}
          />
        </span>
      ))}
...
/**
 * `roboco_loader` (loaders.rs:74-105): the wave loader — 5 cells pulsing
 ...
 */
export function RobocoLoader({ cell = 8, className }: { cell?: number; className?: string }) {
  return (
    <span
      className={`roboco-loader ${className ?? ""}`}
      style={{ "--rb-cell": `${cell}px`, gap: `${cell / 2}px` } as CSSProperties}
      aria-hidden="true"
    >
      {Array.from({ length: ROBOCO_CELLS }, (_, i) => (
        <span
          key={i}
          className="roboco-loader-slot"
          style={{ width: `${cell}px`, height: `${cell}px`, borderRadius: `${cell / 4}px` }}
        >
          <span
            className="roboco-loader-cell"
            style={{ animationDelay: `${i * PULSE_STAGGER * ROBOCO_PULSE_MS}ms` }}
          />
        </span>
      ))}
    </span>
