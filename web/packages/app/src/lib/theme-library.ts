 ...
 *   `theme_library::compile` rides `roboco_theme::vscode` (~1600 lines of
 *   token-color mapping). The web accepts a Roboco-family JSON file
 ...
 *   is one sibling key (`roboco.theme-library.v1`) holding the compiled
...
export const THEME_LIBRARY_STORAGE_KEY = "roboco.theme-library.v1";
...
/**
 * Compile a Roboco-family JSON file into an installable family: every
...
function compileVariant(
  ...
  const base = findVariant(appearance === "dark" ? "roboco-dark" : "roboco-light");
  if (base === undefined) {
