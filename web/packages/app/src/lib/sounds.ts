 ...
 * (`sound.rs:19 ROBOCO_DISABLE_SOUND` → `?disableSound=1`), and playback
...
/**
 * The `ROBOCO_DISABLE_SOUND` / `ROBOCO_DISABLE_NOTIFICATIONS` web
...
/** `ROBOCO_DISABLE_SOUND` equivalent — checked before any playback. */
export function soundDisabled(): boolean {
  return KILL_SWITCH.sound;
}

/** `ROBOCO_DISABLE_NOTIFICATIONS` equivalent — checked before any banner. */
