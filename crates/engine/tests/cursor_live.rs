...
//! ROBOCO_CURSOR_STATE_DIR=$(mktemp -d) cargo test -p roboco-engine --test cursor_live -- --ignored --nocapture --test-threads=1
...
use roboco_harness::CursorHarness;
...
async fn remote_steer_batch_reaches_real_muse_with_every_message() {
    ...
    for (i, token) in tokens.iter().enumerate().skip(1) {
        handle.doc().queue_command(&roboco_doc::SessionCommandEntry {
            id: format!("remote-{i}"),
...
async fn send_now_during_startup_keeps_the_interrupted_user_message() {
    let rounds = std::env::var("ROBOCO_CURSOR_EARLY_ROUNDS")
        .ok()
            ...
            || {
                core.sessions
                    ...
                    .is_some_and(|(events, _)| {
                        events.iter().any(|e| {
                            matches!(e.event, roboco_proto::AgentEvent::SessionStarted { .. })
                        })
                    })
            },
            "shim startup",
        ...
        wait(
            || {
                core.sessions
                    ...
                    .0
                    .iter()
                    .filter(|e| matches!(e.event, roboco_proto::AgentEvent::SessionStarted { .. }))
                    .count()
                    == i + 1
            },
