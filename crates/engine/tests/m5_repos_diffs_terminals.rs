...
async fn git_status_stream_is_scoped_deduplicated_and_resets_after_commit() {
    use roboco_proto::CheckoutGitStatus;
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path().join("repo");
    ...
    let mut stream = client
        ...
        .await
        .unwrap();
    async fn next(stream: &mut roboco_rpc::RpcSubscription) -> CheckoutGitStatus {
        tokio::time::timeout(Duration::from_secs(15), async {
            loop {
                let value = stream.recv().await.expect("stream alive");
                assert!(value.get("patch").is_none());
                if let Some(status) =
                    serde_json::from_value::<roboco_proto::WorkspaceGitStatusFrame>(value)
                        .unwrap()
                        .status
                {
                    return status;
    ...
    assert_eq!(staged.files[0].index, roboco_proto::GitFileState::Modified);
    assert_eq!(
        staged.files[0].worktree,
        roboco_proto::GitFileState::Unchanged
    );
...
async fn project_actions_crud_preserves_saved_actions_with_invalid_imports() {
    ...
    core.workspace
        ...
        .unwrap();
    let client = roboco_rpc::memory_client(core.rpc_service());
    let path = project.join("roboco.json");
...
async fn rpc_dispatch_for_m5_methods() {
    ...
    let setup_command = if cfg!(windows) {
        ...
        concat!(
            "sleep 2; ",
            "printf 'ROOT=%s\\nWT=%s\\nCWD=%s\\n' ",
            "\"$ROBOCO_PROJECT_ROOT\" \"$ROBOCO_WORKTREE_PATH\" \"$PWD\" ",
            "| tee .roboco-setup-env"
        )
        .to_string()
    };
    ...
    use roboco_proto::CreateWorktreeOutcome;
    ...
    assert!(canonical_worktree.join(".roboco-setup-env").exists());
    core.terminals
