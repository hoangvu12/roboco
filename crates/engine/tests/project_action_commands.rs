...
use roboco_engine::project_actions::{
    MAX_PROJECT_ACTION_COMMAND_BYTES, launch_project_action, launch_project_setup_action,
};
use roboco_engine::{ProjectActionsStore, Terminals};
use roboco_proto::{ProjectActionDraft, ProjectActionIcon};
...
async fn manual_and_setup_actions_preserve_long_multiline_commands() {
    ...
    for setup in [false, true] {
        let prefix = "printf '%s' '";
        let suffix = "' > payload\nprintf '%s' \"$ROBOCO_PROJECT_ROOT\" > project-root\nprintf '%s' \"$ROBOCO_WORKTREE_PATH\" > worktree-path\nprintf '%s' 'quotes: \" $() ` ; é' > literal\n";
        let payload =
