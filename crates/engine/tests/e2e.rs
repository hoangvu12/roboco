...
use roboco_engine::{DocHost, DocHostConfig, EngineCore, HarnessRegistry, RunJournal, SessionsEngine, Uploads};
...
async fn generated_image_is_materialized_before_publication_and_survives_reopen() {
    ...
    // Canonical compare: the import publishes the canonicalized uploads root
    // (a `\\?\`-prefixed path on Windows) while `Uploads::dir` is not.
    let uploads_root = uploads.dir().canonicalize().unwrap();
    assert!(std::path::Path::new(&images[0]).starts_with(&uploads_root));
...
async fn real_image_generation_profile_smoke() {
    ...
    let core = EngineCore::assemble(
        dir.path(),
        registry_with(Arc::new(roboco_harness::CodexHarness::new())),
        HarnessId::Codex,
    )
    .unwrap();
    core.sessions
    ...
    // Canonical compare: the import publishes the canonicalized uploads root
    // (a `\\?\`-prefixed path on Windows) while `Uploads::dir` is not.
    let uploads_root = core.uploads.dir().canonicalize().unwrap();
    assert!(std::path::Path::new(image).starts_with(&uploads_root));
