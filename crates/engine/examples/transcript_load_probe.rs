...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    for path in std::env::args().skip(1) {
        ...
        let doc = roboco_doc::SessionDoc::from_doc(doc);
        let t = Instant::now();
        let tail = doc.read_opening_tail(128)?;
        let tail_materialize = t.elapsed();
        let t = Instant::now();
        let preview = roboco_doc::TranscriptUpdate {
            frame: roboco_doc::TranscriptFrame::reset(&tail),
            context_usage: doc.context_usage(),
            replay_baseline: Some(roboco_doc::TranscriptBaseline::capture(&tail)),
        };
        let tail_wire = serde_json::to_vec(&serde_json::to_value(preview)?)?;
        let _: roboco_doc::TranscriptUpdate =
        ...
        let t = Instant::now();
        let entries = roboco_doc::join_continuation_entries(doc.read_entries()?);
        let materialize = t.elapsed();
        let mut kinds = std::collections::BTreeMap::<&str, (usize, usize, usize)>::new();
        for entry in &entries {
            for part in &entry.parts {
                let kind = match part {
                    roboco_doc::MessagePart::Text { .. } => "text",
                    roboco_doc::MessagePart::Reasoning { .. } => "reasoning",
                    roboco_doc::MessagePart::Tool { .. } => "tool",
                    _ => "other",
                };
        ...
        let _: roboco_doc::TranscriptUpdate = serde_json::from_value(value)?;
