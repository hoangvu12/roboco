...
pub(crate) async fn list(
    ...
    let catalog = match result {
        ...
        Some(Ok(Err(error))) if !CatalogFailure::classify(&error).allows_stale() => {
            ...
            if harness.id() == roboco_proto::HarnessId::ClaudeCode
...
mod tests {
    ...
    struct Probe {
        ...
        harness: roboco_proto::HarnessId,
    }
    impl Probe {
        fn new() -> Arc<Self> {
            Arc::new(Self {
                ...
                harness: roboco_proto::HarnessId::Codex,
            })
        }
    }
    ...
    impl Harness for Probe {
        fn id(&self) -> roboco_proto::HarnessId {
            self.harness
        }
        ...
        async fn run(
            &self,
            _: roboco_proto::RunRequest,
            _: roboco_harness::RunControls,
        ) -> Result<
            futures::stream::BoxStream<'static, Result<roboco_proto::AgentEvent, HarnessError>>,
            HarnessError,
        > {
            unreachable!()
        }
    ...
    async fn logged_out_claude_uses_curated_rows_instead_of_disk() {
        ...
        Arc::get_mut(&mut probe).unwrap().harness = roboco_proto::HarnessId::ClaudeCode;
        list(dir.path(), probe.clone(), false).await.unwrap();
        probe.fail.store(true, SeqCst);
