//! Coalesced chat snapshots. The watermark is sampled BEFORE export; snapshot
//! and watermark commit atomically. Queue/lock waits never occupy a Tokio
//! worker. Roboco adaptation: no chat2 room cursor exists engine-locally, so
//! the watermark counts applied generations (`applied`) instead of room rows;
//! production commits simply mark the doc dirty and let the persister
//! coalesce.
...
/// Snapshot-row epoch this persister writes (2 = the roboco doc generation
/// that pairs with `save_snapshot_with_cursor`).
const DOC_EPOCH: u32 = 2;
...
mod tests {
    ...
    /// to it or opens a network connection.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[ignore = "requires ROBOCO_WHALE_SNAPSHOT; reads privately supplied snapshot into a temporary store"]
    async fn real_whale_replay_keeps_146_heartbeats_running_on_two_workers() {
        let bytes = std::fs::read(std::env::var("ROBOCO_WHALE_SNAPSHOT").unwrap()).unwrap();
    ...
    async fn replay_burst_coalesces_and_flush_bypasses_debounce() {
        ...
        assert_eq!(
            ...
            0,
            "a reset watermark must be allowed to decrease"
        );
    }
