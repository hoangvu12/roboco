...
impl Terminals {
    ...
    fn open_session(
        ...
        // ConPTY has no Unix canonical line buffer. Preserve interactive cmd
        // syntax and avoid imposing a PowerShell script execution policy.
        #[cfg(windows)]
        let bootstrap = command.map(|command| format!("{command}\r"));
        #[cfg(windows)]
        let initial_script: Option<tempfile::NamedTempFile> = None;
        #[cfg(windows)]
        ...
        #[cfg(not(windows))]
        let (master, mut child, initial_script, bootstrap) = {
            ...
            if let Some(command) = command {
                let (suffix, source) = match shell_name.as_str() {
                    "fish" => (".fish", "source \"$ROBOCO_ACTION_SCRIPT\"\r"),
                    _ => (".sh", ". \"$ROBOCO_ACTION_SCRIPT\"\r"),
                };
                let mut script = tempfile::Builder::new()
                    .prefix("roboco-action-")
                    .suffix(suffix)
                    .tempfile()?;
                script.write_all(command.as_bytes())?;
                script.flush()?;
                cmd.env("ROBOCO_ACTION_SCRIPT", script.path());
            ...
            drop(pair.slave);
            (pair.master, child, initial_script, bootstrap)
        };
...
mod windows_tests {
    ...
    async fn open_with_command_delivers_the_exact_command_to_conpty() {
        // Windows action delivery writes the command directly (no Unix
        // canonical line buffer); a PowerShell one-liner marker proves the
        // exact text survives the initial write and executes.
        let tmp = tempfile::tempdir().unwrap();
        let marker = tmp.path().join("delivered.txt");
        let command = format!(
            "Set-Content -NoNewline -Path '{}' -Value 'ROBOCO_ACTION_OK'; exit 0",
        ...
        let session = terminals
            ...
            .expect("open PowerShell with an initial command");
        ...
        .await;
        assert_eq!(
            std::fs::read_to_string(&marker).as_deref().ok(),
            Some("ROBOCO_ACTION_OK")
        );
        ...
        assert!(
            terminals
                .open_session(
                    ...
                    80,
                    24,
                    Some("/nonexistent/roboco-test-shell"),
                    &HashMap::new(),
                    Some("echo test"),
                )
                .is_err()
