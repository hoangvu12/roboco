//! Host-local project Actions and explicit `roboco.json` imports.
//!
...
const PROJECT_FILE: &str = "roboco.json";
...
/// Engine-local storage for executable project Actions.
#[derive(Clone)]
...
fn launch_project_action_with_environment(
    ...
    let mut environment =
        HashMap::from([("ROBOCO_PROJECT_ROOT".to_string(), root_string(project_root))]);
    if always_include_worktree || cwd != project_root {
        environment.insert("ROBOCO_WORKTREE_PATH".to_string(), root_string(cwd));
    }
...
fn project_file_issue(message: String) -> (Vec<ProjectActionDraft>, Option<String>) {
    (Vec::new(), Some(format!("Invalid roboco.json: {message}")))
}
...
mod tests {
    ...
    async fn project_actions_rpc_does_not_block_async_worker() {
        use roboco_rpc::{RpcService, methods};
        let temp = tempfile::tempdir().unwrap();
    ...
    fn project_file_special_files_do_not_block() {
        const CHILD_ENV: &str = "ROBOCO_TEST_PROJECT_FILE_SPECIAL_FILES";
