...
//! ROBOCO_CURSOR_STATE_DIR=$(mktemp -d) cargo run -p roboco-harness --example cursor_stability_probe -- sessions 20
//! cargo run -p roboco-harness --example cursor_stability_probe -- models 1000
...
async fn turn(
    ...
    let request = RunRequest {
        ...
        harness: None,
        model: Some(
            std::env::var("ROBOCO_CURSOR_TEST_MODEL").unwrap_or_else(|_| "composer-2.5".into()),
        ),
        reasoning: None,
                    ...
                    match fault {
                        ...
                        "kill" => {
                            let root = PathBuf::from(
                                std::env::var_os("ROBOCO_CURSOR_STATE_DIR")
                            ...
                            let owner: serde_json::Value = serde_json::from_slice(
                                &std::fs::read(PathBuf::from(dir.trim()).join(".roboco-owner.json"))
                                    .unwrap(),
                            )
                            .unwrap();
                            assert!(
                ...
                AgentEvent::Done{status,error,..}=>{
                    ...
                    if completed==2 {
                        if let Some(before)=auth_exchanges_before {
                            let control=std::env::var("ROBOCO_CURSOR_AUTH_CLOCK").unwrap();
                    ...
                    if completed==1 {
                        if let Ok(control)=std::env::var("ROBOCO_CURSOR_AUTH_CLOCK") {
                    ...
                    tx.send(roboco_harness::SteerMessage{prompt:"Repeat the exact PARKED-STABILITY token from earlier. Reply only the token. Do not use tools or files.".into(),message_id:None}).await.unwrap();
                }
                _=>{}
            }
        }
...
async fn history(harness: &CursorHarness, count: usize) {
    ...
    let request = RunRequest {
        ...
        harness: None,
        model: Some(
            std::env::var("ROBOCO_CURSOR_TEST_MODEL").unwrap_or_else(|_| "muse-spark-1.3".into()),
        ),
        reasoning: None,
        model_options: Default::default(),
        cwd: workspace.path().to_str().unwrap().into(),
    ...
    let producer = tokio::spawn(async move {
        ...
        for token in sent.iter().skip(1) {
            tx.send(roboco_harness::SteerMessage{prompt:format!("Add token {token} to your remembered conversation history. Reply with the token from the immediately previous user message, followed by this new token. Do not use tools."),message_id:Some(uuid::Uuid::new_v4().to_string())}).await.unwrap();
        }
    });
    let mut id = String::new();
    let mut text = String::new();
    ...
    if mode == "resolve" {
        ...
        let outage = if mode == "outage" {
            let flag = PathBuf::from(
                std::env::var_os("ROBOCO_CURSOR_STRESS_OUTAGE_FLAG")
        ...
        assert!(std::env::var_os("ROBOCO_CURSOR_STATE_DIR").is_some());
        ...
        assert!(
            std::env::var_os("ROBOCO_CURSOR_STATE_DIR").is_some(),
            "set an isolated ROBOCO_CURSOR_STATE_DIR"
        );
        let workspace = tempfile::tempdir().unwrap();
