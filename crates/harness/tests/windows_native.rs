    ...
    for arg in arguments {
        let output = tokio::time::timeout(
            ...
            Command::new(&script)
                .arg(arg)
                .current_dir(dir.path())
                .env("ROBOCO_BATCH_ATTACK", "EXPANDED&echo injected>injected.txt")
                .output(),
        )
        .await
        .unwrap()
...
async fn batch_executable_path_rejects_percent_expansion() {
    let dir = tempfile::tempdir().unwrap();
    let script = dir.path().join("shim%ROBOCO_BATCH_NAME%.cmd");
    std::fs::write(&script, "@exit /b 0\r\n").unwrap();
    assert_eq!(
        roboco_harness::process::Command::new(&script)
            .spawn()
            .unwrap_err()
            .kind(),
        std::io::ErrorKind::InvalidInput
