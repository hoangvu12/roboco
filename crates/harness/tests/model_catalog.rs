...
use roboco_harness::{AcpHarness, CodexHarness, Harness};
...
async fn every_native_catalog_retains_last_good_and_cold_failure_stays_an_error() {
    ...
    for harness in harnesses(&binary) {
        ...
        assert_eq!(
            roboco_harness::CatalogFailure::classify(&error),
            roboco_harness::CatalogFailureCode::AuthRequired
        );
        assert!(
...
async fn codex_empty_catalogs_retire_children_and_next_request_spawns_fresh() {
    ...
    assert_eq!(
        roboco_harness::CatalogFailure::classify(&error),
        roboco_harness::CatalogFailureCode::Failed
    );
...
fn auth_context_child() {
    let Some(root) = std::env::var_os("ROBOCO_MODEL_CONTEXT_TEST_ROOT") else {
        return;
    };
    ...
    let claude = roboco_harness::ClaudeHarness::new().with_executable(&binary);
    let opencode = roboco_harness::OpencodeHarness::new().with_executable(&binary);
    for (harness, file) in [
...
fn each_spec_hashes_its_auth_file_contents() {
    ...
    command
        ...
        .env("HOME", dir.path())
        .env("ROBOCO_MODEL_CONTEXT_TEST_ROOT", dir.path());
    for key in [
        "CLAUDE_CONFIG_DIR",
        "XDG_CONFIG_HOME",
        "OPENCODE_CONFIG",
