...
impl SessionFixture {
    fn new() -> Self {
        ...
        .unwrap();
        let source = std::env::var_os("ROBOCO_CURSOR_TEST_SHIM")
    ...
    async fn start(
        ...
        let mut child = tokio::process::Command::new("node")
            ...
            .env("ROBOCO_CURSOR_STATE_DIR", self.dir.path().join("state"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .kill_on_drop(true)
...
async fn engine_death_does_not_leave_an_orphan_owning_the_conversation() {
    ...
    let mut engine = tokio::process::Command::new("node")
        ...
        .env("ROBOCO_CURSOR_STATE_DIR", fixture.dir.path().join("state"))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
        .spawn()
...
async fn repeated_startup_failures_retain_all_user_messages_without_nesting_or_duplicates() {
    ...
    let receipt: serde_json::Value = serde_json::from_slice(
        &std::fs::read(std::path::Path::new(store.trim()).join(".roboco-user-receipt.json"))
            .unwrap(),
    )
    .unwrap();
...
async fn corrupt_interrupted_receipt_fails_before_sending_a_contextless_prompt() {
    ...
    std::fs::write(root.join(".roboco-user-receipt.json"), "{\"version\":999}").unwrap();
