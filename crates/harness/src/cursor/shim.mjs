...
async function claimStore(local) {
  const marker = path.join(local.dir, ".roboco-owner.json");
  let previous;
...
async function recoverInterruptedRun(agentId) {
  ...
  if (interruptedRun && ["queued", "running"].includes(interruptedRun.status)) {
    await ownedStore.runs.update({run: {
      ...
      error: "The previous Roboco process stopped before completing this turn.",
    }});
  }
...
async function start(msg) {
  ...
  if (runDir) {
    ...
    receiptPath = path.join(runDir, ".roboco-user-receipt.json");
  }
