...
/// The pinned SDK (public beta 1.0.x line; inspected against 1.0.32's
...
impl Harness for CursorHarness {
    ...
    async fn run(
        ...
        cmd.args(&args);
        if lease.is_some() {
            cmd.env("ROBOCO_CURSOR_STATE_DIR", state::state_root());
        }
        crate::compose_child_path(&mut cmd, &exe);
                ...
                Ok(Some(line)) => {
                    ...
                    match frame.get("ev").and_then(Value::as_str).unwrap_or("") {
                        ...
                        _ => {
                            ...
                            {
                                tracing::warn!(target: "roboco_harness::cursor",
