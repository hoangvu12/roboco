...
/// sits better with a smaller apparent gap.
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let container = div()
            ...
            .when_some(failure, |el, message| {
                ...
                // stroke. Amber with "Warning" for the offline-ish case
                // (engine not connected), red with "Error" for send/run
                // failures. Click dismisses.
...
mod tests {
    ...
    fn enter_on_empty_composer_during_a_live_run_never_interrupts(cx: &mut gpui::TestAppContext) {
        ...
        // If Enter had dispatched an interrupt, the composer task would have
        // handed a QueueCommand call to the test engine, writing its frame
        // into this channel by the time the executors park.
        cx.run_until_parked();
