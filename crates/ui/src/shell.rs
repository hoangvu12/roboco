...
impl Shell {
    ...
    fn render_chat_row(
        ...
        let show_label = self.settings.sidebar_show_project_label;
        let remote = self
            .state
            .read(cx)
            .chats
        ...
        div()
            ...
            // space-name matches can highlight; the sidebar honors compact
            // and the show-label preference.
            .when(!compact && show_label || search_query.is_some(), |el| {
                el.child(
                    div()
                        .w_full()
                        .flex()
