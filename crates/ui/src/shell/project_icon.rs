...
fn load_local_icon(root: &std::path::Path) -> Option<MediaImage> {
    ...
    for path in local_icon_candidates(root) {
        ...
        let bytes = std::fs::File::open(&path).ok().and_then(|file| {
            ...
            file.take(roboco_proto::MAX_WORKSPACE_IMAGE_BYTES as u64 + 1)
...
impl ProjectIcon {
    fn new(
        ...
        engine: Option<crate::engine_registry::EngineTarget>,
        cx: &mut Context<Self>,
    ) -> Self {
        ...
        let task = cx.spawn(async move |this, cx| {
            ...
            let load = async {
                ...
                for query in ["icon", "logo"] {
                    let matches = client
                        .search(roboco_proto::SearchWorkspaceFilesRequest {
                            target: context.target.clone(),
                            query: query.into(),
                    ...
                    candidates.extend(matches.into_iter().filter_map(|entry| {
                        (entry.kind == roboco_proto::WorkspaceEntryKind::File)
    ...
    pub(super) fn render_project_icon(
        ...
        let context = space.map(|space| FilesRequestContext {
            engine: engine
                ...
                .unwrap_or_else(crate::engine_registry::EngineKey::local),
