//! Shell glue for host-owned project Actions: the titlebar control, its menu,
//! the editor overlay, and the worktree-setup terminal attach. The owning
//! engine's connection is resolved once per action context; the engine
//! asserts ownership fail-closed (see `local_project_action_space`).
...
impl Shell {
    ...
    /// The chat's space and the engine connection that owns it. The projected
    /// chat/space rows carry engine-scoped ids; `target_for_id` resolves the
    /// owning connection, and `wire_params` decodes the scoped ids back to the
    /// engine's raw ids at the socket (stripping `targetDeviceId`).
    fn project_action_context(&self, cx: &App) -> Option<ProjectActionContext> {
        ...
        let engine = state.target_for_id(&chat.device_id).ok()?;
    ...
    fn render_project_actions_menu(
        ...
        if !snapshot.importable_actions.is_empty() {
            card = card
                ...
                .child(popover::menu_heading(&theme, "Import from roboco.json"));
        ...
        if editor.confirm_delete {
            ...
            let card = popover::dialog_card(&theme)
                ...
                .child(div().mt(px(6.0)).child(popover::dialog_body(
                    &theme,
                    format!("\u{201C}{name}\u{201D} will be permanently deleted."),
                )))
                .child(
                    div()
                        .mt(px(16.0))
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                popover::btn_primary(
                                    &theme,
                                    if saving { "Saving\u{2026}" } else { "Save action" },
                                )
                                .id("action-save")
                                .when(!saving, |button| {
                                    button.on_click(
