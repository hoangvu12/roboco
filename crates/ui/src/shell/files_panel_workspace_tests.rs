...
//! Set ROBOCO_FILES_CAPTURES to a directory to run on X11 and capture the fixture.
use super::*;
...
fn files_panel_workspace_navigation_and_external_updates() {
    ...
    drop(listener);
    let _ipc = runtime
        .block_on(roboco_engine::serve_ipc(port, core.rpc_service()))
        .unwrap();
    let output = std::env::var_os("ROBOCO_FILES_CAPTURES").map(PathBuf::from);
