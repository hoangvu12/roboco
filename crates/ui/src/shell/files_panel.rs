...
impl Shell {
    ...
    pub(super) fn on_files_panel_drag(
        ...
        let requested = viewport - f32::from(event.event.position.x);
        let sample = motion::resize_drag_sample(
            requested,
            FILES_PANEL_MIN,
            FILES_PANEL_MAX,
            self.files_resize_edge,
            self.reduced_motion,
        );
        self.settings.files_panel_width = sample.width;
        self.pane_resize_dragging = Some(PaneResizeKind::Files);
        self.pane_resize_active = sample.edge.is_none().then_some(PaneResizeKind::Files);
        self.files_resize_edge = sample.edge;
    ...
    pub(super) fn render_files_panel(&mut self, _window: &Window, cx: &mut Context<Self>) -> AnyElement {
