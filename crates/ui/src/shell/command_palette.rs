...
impl Shell {
    ...
    pub(super) fn render_command_palette(
        ...
        for (ix, entry) in entries.iter().enumerate() {
            ...
            let content = if let Some((label, glyph)) = entry.action() {
                ...
                self.render_chat_row(
                    ...
                    ix == active,
                    chat.archived,
                    // The palette's copy is never a drag preview, carries no
                    // reorder payload, and shows no jump chip.
                    false,
                    None,
                    None,
                    Some(&query),
        ...
        // The shared popup glass: the same frosted mount as every menu and
        // dialog (roboco's MENU_BLUR), with its opaque fallback when frost is
        // disabled.
...
mod tests {
    ...
    fn search_matches_words_across_chat_metadata() {
        assert!(matches_query(
            "mac auth",
            "Fix authentication Roboco @ MacBook main"
        ));
        assert!(matches_query("  ", "Any chat"));
        assert!(!matches_query("mac windows", "Roboco @ MacBook"));
    }
}
