...
pub(crate) struct TranscriptPreparation {
    ...
    baseline: Option<roboco_doc::TranscriptBaseline>,
}
...
pub(crate) struct PreparedTranscript {
    ...
    pub(crate) navigation_baseline: Arc<roboco_doc::TranscriptBaseline>,
...
impl TranscriptPreparation {
    pub(crate) fn prepare(
    ...
    ) -> Result<Arc<PreparedTranscript>, roboco_doc::TranscriptDesync> {
        match &update.frame {
            roboco_doc::TranscriptFrame::Reset { .. } => {
                self.cache.clear();
                self.tree_cache.clear();
                self.live_parsers.clear();
            }
            roboco_doc::TranscriptFrame::Delta {
                upsert,
                append,
                remove,
                ..
        ...
        roboco_doc::apply_transcript_frame(&mut self.entries, update.frame.clone())?;
        ...
        Ok(Arc::new(PreparedTranscript {
            ...
            fully_historical,
            bytes,
            navigation_baseline: Arc::new(roboco_doc::TranscriptBaseline::capture(&self.entries)),
        }))
    }
}
...
pub struct Transcript {
    ...
    last_replay_baseline: Option<Arc<roboco_doc::TranscriptBaseline>>,
...
impl Transcript {
    ...
    fn toggle_fold(
        ...
        // Ticket 71 A — the anchor is captured at the click position (the
        // rows have not re-split yet), for shells and ordinary folds alike.
...
mod tests {
    ...
    fn tool_group_revisit_skips_batched_history_but_animates_live_arrivals(
        ...
        with_tool_group_navigation(cx, |state, transcript, cx| {
            ...
            apply_frame(
                ...
                Some(roboco_doc::TranscriptBaseline::capture(&cached)),
                cx,
            );
            ...
            for (previous, next) in [(&cached, &first_batch), (&first_batch, &second_batch)] {
                ...
                apply_frame(
                    frame,
                    Some(roboco_doc::TranscriptBaseline::capture(next)),
                    cx,
                );
                let reveal = &transcript.read(cx).tool_group_reveals[&row_id];
    ...
    fn prepared_whale_open_and_revisit_do_not_build_rows_on_ui(cx: &mut gpui::TestAppContext) {
        let (update, prepared, preparation_ms) = std::thread::spawn(|| {
            let entries = if let Ok(path) = std::env::var("ROBOCO_WHALE_SNAPSHOT") {
                let doc = roboco_doc::SessionDoc::init("fixture").unwrap();
                doc.doc().import(&std::fs::read(path).unwrap()).unwrap();
                roboco_doc::join_continuation_entries(doc.read_entries().unwrap())
            } else {
    ...
    fn tool_group_replay_cutoff_survives_coalesced_live_updates(cx: &mut gpui::TestAppContext) {
        with_tool_group_navigation(cx, |state, transcript, cx| {
            ...
            state.update(cx, |state, cx| {
                let frame = roboco_doc::diff_transcript(&state.transcript, &history);
                state
                    .receive_transcript_update(
                        roboco_doc::TranscriptUpdate {
                            frame,
    ...
    fn tool_group_interleaved_history_preserves_live_animation_epochs(
        ...
        with_tool_group_navigation(cx, |state, transcript, cx| {
            let apply = |entries: &[SessionMessageEntry], baseline, cx: &mut gpui::App| {
                state.update(cx, |state, cx| {
                    let frame = roboco_doc::diff_transcript(&state.transcript, entries);
                    state
                        .receive_transcript_update(
                            roboco_doc::TranscriptUpdate {
                                frame,
            ...
            apply(
                &entries,
                Some(roboco_doc::TranscriptBaseline::capture(&historical)),
                cx,
            );
    ...
    fn tool_group_live_reset_does_not_become_history(cx: &mut gpui::TestAppContext) {
        with_tool_group_navigation(cx, |state, transcript, cx| {
            ...
            state.update(cx, |state, cx| {
                let frame = roboco_doc::diff_transcript(&state.transcript, &live);
                assert!(matches!(&frame, roboco_doc::TranscriptFrame::Reset { .. }));
                state
                    .receive_transcript_update(
                        roboco_doc::TranscriptUpdate {
                            frame,
    ...
    fn compact_fold_keeps_body_mounted_through_the_close_tween(cx: &mut gpui::TestAppContext) {
        ...
        with_tool_group_navigation(cx, |state, transcript, cx| {
            ...
            transcript.update(cx, |this, cx| {
                this.toggle_fold(work_id.clone(), 0, 0.0, 0.0, false, cx);
            ...
            with_window(|transcript, window, cx| {
                ...
                for pinned in [false, true] {
                    transcript.update(cx, |this, cx| {
                        ...
                        this.state.update(cx, |state, cx| {
                            let frame = roboco_doc::diff_transcript(&state.transcript, &updated);
                            state
                                .receive_transcript_update(
                                    roboco_doc::TranscriptUpdate {
                                        frame,
