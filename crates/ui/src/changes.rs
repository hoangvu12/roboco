...
";
    ...
    /// The headless Windows platform swaps in a NoopTextSystem with uniform
    /// stub metrics (every family resolves mono at 0.6em), which defeats the
    /// proportional-font premise; Windows needs the non-headless platform's
    /// real DirectWrite stack. macOS/Linux headless keep their real text
    /// systems, so upstream's `true` stays there.
    #[test]
    fn native_diff_font_geometry() {
        let platform = gpui_platform::current_platform(!cfg!(target_os = "windows"));
        let text_system =
