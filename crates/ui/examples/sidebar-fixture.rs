...
fn main() -> anyhow::Result<()> {
    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        settings.surface = roboco_theme::SurfacePreference::Frosted;
        ...
        appearance::init(if std::env::var_os("ROBOCO_PALETTE_LIGHT").is_some() { appearance::AppearanceMode::Light } else { appearance::AppearanceMode::Dark }, settings.theme_selection, settings.accent, settings.surface, cx);
        ...
        let state = cx.new(|_| {
            ...
            for ix in [1, 2, 4] {
                ...
                s.fixture_sidebar_change_request(roboco_proto::CheckoutChangeRequestStatus {
                    ...
                    change_request: Some(roboco_proto::ChangeRequestSummary { provider: "github".into(), number: 412 + ix as u64, title: chat.title.clone().unwrap(), url: "https://github.com/zeronsh/zeron/pull/412".into(), state: roboco_proto::ChangeRequestState::Open, base_ref: "main".into(), head_ref: source.branch.clone() }),
                });
            }
