...
Run `cargo run -p roboco-ui --example command-palette-fixture --features command-palette-fixture` on a desktop, then
press Cmd+K (Ctrl+K on Linux/Windows). Set `ROBOCO_PALETTE_LIGHT=1` for light mode.

Verified interactively on Linux/X11 (upstream record, zeron 1400f7ab):
...
The palette uses the shared popup glass surface (roboco's frosted mount and
MENU_BLUR, with the opaque fallback when frost is disabled). The
platform-specific Cmd key binding has not been exercised on macOS in this
Linux environment.
