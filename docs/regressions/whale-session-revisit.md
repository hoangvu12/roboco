...
Ported from zeronsh/zeron (75850fad, 776adb6f, 57a3a7c1). Upstream base:
`c24e0e26` (main, v0.2.76). Verified with synthetic local data; this is not a
claim that any particular production session or installed application was
tested.
...
account replacement, and a persisted 2,000-message snapshot opened with no
network dependency. One debug run measured 36.47 ms for offline cold open and
35.06 ms for rebuilding the cleared mirror; these are observations, not timing
gates.

## Reproduce

```sh
cargo test -p roboco-engine --lib -- --nocapture
cargo test -p roboco-ui --lib --no-default-features -- --test-threads=1
```
...
The revisit fixes above did not address the first-open cost. A read-only copy
of the locally available **Zlang Performance And Safety** snapshot was profiled
without any network connection. No transcript content or snapshot is committed.
...
```sh
cargo run -p roboco-engine --example transcript_load_probe -- /path/to/session.bin
```

The new path removes the full-history barrier to initial content. It does not
claim that reconstructing/rendering all 37,630 parts is instantaneous.
...
No animation behavior change was needed; both opening frames carry historical
part baselines, and cache restoration captures its own baseline before
presentation.
